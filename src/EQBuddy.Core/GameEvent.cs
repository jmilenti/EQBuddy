...
/// <param name="Critical">The heal carried a trailing "(Critical)" note, exactly as
/// critical HITS do. <param name="Note">is the note verbatim, so a shape we don't
/// classify yet ("Riposte Critical", a future heal annotation) still reaches the UI
/// instead of being flattened to a bool.</param></param>
public record HealEvent(DateTime Time, string Target, int Amount, string Spell, bool Outgoing, string Healer = "", bool OverTime = false, bool Critical = false, string? Note = null) : GameEvent(Time);
